package org.royaldev.storage.commands;
...
public abstract class BaseCommand<T extends Plugin> implements CommandExecutor {
     ...
     *
     * @param instance         The instance of Storage that this command is being registered with
     * @param name             The name of this command
     * @param checkPermissions Whether to check permissions using the auto-generated permission
     */
    public BaseCommand(final T instance, final String name, final boolean checkPermissions) {
    ...
    private void showFlagHelp(final CommandSender cs, final Command cmd, final String label) {
        ...
        cs.sendMessage(ChatColor.BLUE + "Expected flags:");
        for (final Flag f : this.getExpectedFlags()) {
            String message = "  " + ChatColor.GRAY + "-" + f.getNames();
     ...
     */
    public void dispNoPerms(final CommandSender cs) {
        cs.sendMessage(ChatColor.RED + "You don't have permission for that!");
        this.plugin.getLogger().warning(cs.getName() + " was denied access to that!");
    }
     ...
     */
    public void dispNoPerms(final CommandSender cs, final String message) {
        cs.sendMessage(message);
        this.plugin.getLogger().warning(cs.getName() + " was denied access to that!");
    }
    ...
    protected void handleException(final CommandSender cs, final Command cmd, final String label, final String[] args, final Throwable t, final String message) {
        new FancyMessage(message).color(ChatColor.RED).send(cs);
        t.printStackTrace();
        final StringBuilder sb = new StringBuilder();
        sb
    ...
    public final boolean onCommand(final CommandSender cs, final Command cmd, final String label, final String[] args) {
        ...
        final String permission = this.plugin.getDescription().getName().toLowerCase() + "." + cmd.getName();
        if (this.checkPermissions && !cs.hasPermission(permission)) {
            this.dispNoPerms(cs, new String[]{permission}); // ensure calling to varargs method
            return true;
        }
        ...
        if (expectedFlags.size() > 0) {
            for (final Flag f : new CommandArguments(args)) {
                if (!expectedFlags.contains(f)) {
                    ...
                    cs.sendMessage(ChatColor.RED + "Unexpected flag \"" + f.getFirstName() + ".\"");
                    return true;
                }
            }
        }
    ...
    protected class CommandArguments extends ArrayList<Flag> {
         ...
         * @param <F>    Type to return (will cast)
         * @return Converted value
         */
        @SuppressWarnings("unchecked")
        private <F> F convertFlag(final String[] values, final Class<F> clazz) {
            ...
            if (String[].class.isAssignableFrom(clazz)) {
                ...
                return (F) (Integer) Integer.parseInt(joined);
            } else if (Short.class.isAssignableFrom(clazz)) {
                return (F) (Short) Short.parseShort(joined);
            } else if (Long.class.isAssignableFrom(clazz)) {
                return (F) (Long) Long.parseLong(joined);
            } else if (Float.class.isAssignableFrom(clazz)) {
                return (F) (Float) Float.parseFloat(joined);
            }
            return null;
        }
         ...
         */
        public <F> Flag<F> getFlag(final Flag<F> flag) {
